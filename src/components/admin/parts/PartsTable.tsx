import React from 'react';
import { Edit, Wrench } from 'lucide-react';
import type { Part } from '@/types/parts';
import { Skeleton } from '@/components/ui/skeleton';

interface PartsTableProps {
  parts: Part[];
  isLoading: boolean;
  onEdit: (part: Part) => void;
}

export const PartsTable: React.FC<PartsTableProps> = ({
  parts,
  isLoading,
  onEdit,
}) => {
  return (
    <div className="bg-surface border border-outline-variant rounded-xl overflow-hidden shadow-sm flex-1">
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-surface-container-low border-b border-outline-variant">
              <th className="py-3 px-4 font-label-md text-label-md text-on-secondary-fixed-variant uppercase tracking-wider w-16 text-center">Ícono</th>
              <th className="py-3 px-4 font-label-md text-label-md text-on-secondary-fixed-variant uppercase tracking-wider">Nombre del Repuesto</th>
              <th className="py-3 px-4 font-label-md text-label-md text-on-secondary-fixed-variant uppercase tracking-wider">SKU</th>
              <th className="py-3 px-4 font-label-md text-label-md text-on-secondary-fixed-variant uppercase tracking-wider text-center">Stock</th>
              <th className="py-3 px-4 font-label-md text-label-md text-on-secondary-fixed-variant uppercase tracking-wider text-right">Precio Compra</th>
              <th className="py-3 px-4 font-label-md text-label-md text-on-secondary-fixed-variant uppercase tracking-wider text-right">Precio Venta</th>
              <th className="py-3 px-4 font-label-md text-label-md text-on-secondary-fixed-variant uppercase tracking-wider text-center w-24">Acciones</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-outline-variant/50">
            {isLoading ? (
              // Loading State
              Array.from({ length: 5 }).map((_, index) => (
                <tr key={index} className="hover:bg-surface-container-lowest transition-colors">
                  <td className="py-2 px-4 text-center">
                    <Skeleton className="w-10 h-10 rounded bg-surface-container mx-auto" />
                  </td>
                  <td className="py-3 px-4">
                    <Skeleton className="h-5 w-3/4 bg-surface-container" />
                  </td>
                  <td className="py-3 px-4"><Skeleton className="h-4 w-24 bg-surface-container" /></td>
                  <td className="py-3 px-4 text-center"><Skeleton className="h-4 w-8 bg-surface-container mx-auto" /></td>
                  <td className="py-3 px-4 text-right"><Skeleton className="h-4 w-16 bg-surface-container ml-auto" /></td>
                  <td className="py-3 px-4 text-right"><Skeleton className="h-4 w-16 bg-surface-container ml-auto" /></td>
                  <td className="py-3 px-4"><Skeleton className="h-8 w-12 bg-surface-container mx-auto" /></td>
                </tr>
              ))
            ) : parts.length === 0 ? (
              // Empty State
              <tr>
                <td colSpan={7} className="py-12 text-center text-on-surface-variant">
                  <div className="flex flex-col items-center justify-center">
                    <Wrench className="w-12 h-12 text-outline mb-2" />
                    <p className="font-medium text-lg text-on-surface">No se encontraron repuestos</p>
                    <p className="text-sm mt-1">Ajusta los filtros o crea un nuevo repuesto.</p>
                  </div>
                </td>
              </tr>
            ) : (
              // Data Rows
              parts.map((part) => {
                const isLowStock = part.stock > 0 && part.stock <= 5;
                const isOutOfStock = part.stock === 0;

                return (
                  <tr
                    key={part.id}
                    className={`hover:bg-surface-container-lowest transition-colors group ${isLowStock ? 'bg-error-container/10' : ''
                      } ${isOutOfStock ? 'opacity-70' : ''
                      }`}
                  >
                    <td className="py-2 px-4 text-center align-middle">
                      <div className="w-10 h-10 rounded bg-surface-container-highest border border-outline-variant flex items-center justify-center overflow-hidden shrink-0 mx-auto">
                        <Wrench className="w-5 h-5 text-outline" />
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <p className={`font-body-md text-body-md font-medium text-on-surface ${isOutOfStock ? 'line-through text-on-surface-variant' : ''}`}>
                        {part.name}
                      </p>
                    </td>
                    <td className="py-3 px-4">
                      <span className="font-data-mono text-data-mono text-secondary">{part.sku}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className={`font-data-mono text-data-mono font-semibold ${isOutOfStock ? 'text-error line-through' : isLowStock ? 'text-error' : 'text-on-surface'
                        }`}>
                        {part.stock}
                      </span>
                      {isOutOfStock && <span className="block text-[10px] text-error font-medium">Agotado</span>}
                      {isLowStock && <span className="block text-[10px] text-error font-medium">Bajo</span>}
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span className="font-data-mono text-data-mono text-on-surface-variant">
                        ${part.purchasePrice.toFixed(2)}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span className="font-data-mono text-data-mono text-on-surface font-semibold">
                        ${part.salePrice.toFixed(2)}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <div className="flex justify-center items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => onEdit(part)}
                          className="p-1.5 text-on-surface-variant hover:text-primary hover:bg-primary/10 rounded transition-colors"
                          title="Editar"
                        >
                          <Edit className="w-[18px] h-[18px]" />
                        </button>
                      </div>
                    </td>
                  </tr>
                );
              })
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
};
